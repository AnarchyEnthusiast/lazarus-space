...
minetest.register_on_generated(function(minp, maxp, blockseed)
	...
	local has_surface   = (maxp.y >= SURFACE_BASE and minp.y <= UPPER_ASTEROID_MIN - 95)
	local has_upper_ast = (minp.y <= UPPER_ASTEROID_MAX + 25 and maxp.y >= UPPER_ASTEROID_MIN - 95)
	...
	for z = minp.z, maxp.z do
		for y = minp.y, maxp.y do
			for x = minp.x, maxp.x do
				...
				elseif y >= SURFACE_BASE and y < UPPER_ASTEROID_MIN - 95 and has_surface then
					-- Surface Biome Zone - handled per-column below

				elseif y >= UPPER_ASTEROID_MIN - 95 and y <= UPPER_ASTEROID_MAX + 25 and has_upper_ast then
					...
					if not in_hollow then
						...
						elseif y < UPPER_ASTEROID_MIN then
							-- Below zone floor: only hollow asteroid geometry extends here
							if not in_hollow then
								data[vi] = c.air
							end
						else
	...
	if has_caves or has_upper_ast then
		...
		local ast_y_min   = has_upper_ast and math_max(minp.y + 1, UPPER_ASTEROID_MIN - 95) or 0
	...
	if has_surface and #lazarus_space.bio_surface_biomes > 0 then
		...
		local surf_y_max = math_min(maxp.y, UPPER_ASTEROID_MIN - 96)
	...
	if has_surface then
		...
		local cleanup_max_y = math_min(maxp.y, UPPER_ASTEROID_MIN - 96)
